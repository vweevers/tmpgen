...
function del(t, tmp) {
  ...
  var nonExistent = paths.filter(function (p) {
...
test('basic', function (t) {
  var tmp = tmpgen('tmpgen-test-basic-*', { gen: genspy() });
...
test('only last wildcard is dynamic', function (t) {
  ...
  var tmp = tmpgen('tmpgen-test-dynamic/*/*', { gen: gen });
  var dir = join(TMP, 'tmpgen-test-dynamic');
...
test('subdir', function (t) {
  var tmp = tmpgen('tmpgen-test-subdir/*');
...
test('many dirs', function (t) {
  var tmp1 = tmpgen('tmpgen-test-many1/*'),
      paths1 = [];
  var tmp2 = tmpgen('tmpgen-test-many2/*', { gen: 'hat' }),
      paths2 = [];
...
test('resolves paths', function (t) {
  var tmp = tmpgen('tmpgen-test-resolve/foo/../foo-*/../bar-*', { gen: genspy() });
...
test('throws if no wildcard and dir exists', function (t) {
  var tmp1 = tmpgen('tmpgen-test-no-wildcard/*'),
      p1 = tmp1(),
      name = basename(p1);

  t.ok(existent.sync(p1), debug(p1));

  var tmp2 = tmpgen('tmpgen-test-no-wildcard/' + name);
  t.throws(tmp2);
...
test('factory takes additional path', function (t) {
  var tmp = tmpgen('tmpgen-additional-path/*', { gen: genspy('hat') });
  var dir = join(TMP, 'tmpgen-additional-path');
...
test('deletes subfolder of created dir', function (t) {
  var tmp = tmpgen('tmpgen-del-subfolder/*'),
      p1 = tmp(),
      sub = join(p1, 'sub');
...
test('cleans on process exit', function (t) {
  ...
  child.on('close', function (code) {
    ...
    var parent = dirname(paths[0]);
    t.notOk(existent.sync(parent), 'deleted parent: ' + parent);
  });
});

test('cleans on process exit with code', function (t) {
  t.plan(3);

  var child = spawn('node', [join(__dirname, 'scripts', 'clean-with-exitcode.js')]);
  var path = undefined;
...
test('cleans on process exit with error', function (t) {
  t.plan(3);

  var child = spawn('node', [join(__dirname, 'scripts', 'clean-with-error.js')]);
  var path = undefined;
