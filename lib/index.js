...
var isAbsolute = _require.isAbsolute;
...
function tmpgen(path, opts) {
  ...
  fn.del = function (path) {
    ...
    if (path === undefined) {
      ctx.paths = ctx.paths.filter(function (p) {
        return !del(p);
      });
    } else if (typeof path !== 'string') {
      throw new Error('tmpgen: expected a string path, got ' + kindOf(path));
    } else {
      for (var i = 0, l = ctx.paths.length; i < l; i++) {
        var resolved = resolve(ctx.paths[i], path);
      ...
      throw new Error('tmpgen: this instance did not create path "' + path + '" or an ancestor');
    }
...
function pathFactory(path, segments, opts, root) {
  ...
  while (segments.length) {
    ...
    if (dynamic === null) {
      var made = recordMkdir(baseString, this.paths);
    ...
    for (var i = 0; i <= MAX_REPEAT && p === null; i++) {
      ...
      var res = make(baseString, dynamic, i || 1, opts.gen, p, this.paths);
      p = res.created;
